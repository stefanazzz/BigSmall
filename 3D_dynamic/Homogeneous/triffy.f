...
c and paraxial absorbing boundaries on the three others.
c for simulation of an infinite medium
c *************************
c Stefan Nielsen September 2000
c Updated October 2024 to allow for spontaneous quasi-static nucleation
c
c PILOT
c
...
c       open(unit=10, file="filtered_array_fortran.txt", action='read')
c       read(10, *) strini
c       close(10)
c     strini = strini/100.
...
c case flowerlike
c         xpo=float(i-nxf1)*dx;ypo=float(j-nyf1)*dx
c         raa=sqrt(xpo**2+ypo**2)
c         if (ypo.eq.0) then
c            co=1.
c         else
c            co=xpo/ypo
...
c         endif
c        gam=atan(si/co)
c        co=(cos(gam));si=(sin(gam))
c        xp=float(i-nxf1);yp=float(j-nyf1);
c        elrad=dx*sqrt((xp/sqrt(3.0))**2+yp**2)
c        if ( (raa. lt. sqrt(co)*RadiusInit)
c      & .or.(raa . lt.  sqrt(si)*RadiusInit) ) then
c        !if (((nxf1-i)**2+(nyf1-j)**2)*dx.lt.(RadiusInit)**2) then
c        !if (elrad.le.RadiusInit) then
c             peak_31(i,j)=(s31(i,j,nzf)+strini(i,j))*.99
c             broken(i,j)=1
c             rdis(i,j)=0.
c             nbr=nbr+1
c             write (37,*) i,j,it,nbr
c         !strini(i,j)=peak*1.05;else;strini(i,j)=ssini
c         !peak_31(i,j)=(s31(i,j,nzf)+strini(i,j))*.99
c         !broken(i,j)=1
c        endif
c       enddo
...
c       call flush(37)
c case ellipse
        a_ell=sqrt(RadiusInit**2)/0.9194
        b_ell=a_ell*0.9194
        xpo=float(i-nxf1)*dx;ypo=float(j-nyf1)*dx
        raa=sqrt(xpo**2/a_ell**2+ypo**2/b_ell**2)
        if (raa. lt. 1.0) then
...
c           friction=dd*peak_31(i,j)+(1.-dd)*(mu_d*rns)
            ...
            if (iasp(i,j).eq.1 .and. newfail.eq.0) then
              if (ddt.gt.dd) then
              friction=ddt*peak_31(i,j)
              endif
            endif
