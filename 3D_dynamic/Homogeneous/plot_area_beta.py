...
##############################
os.chdir('/home/lcbt87/BigSmall/Homogeneous3/RES')
...
######################################################
#### Read, process and plot rupture fronts from FIRST folder
######################################################
...
ax.plot(rupxy[:1150,1],rupxy[:1150,0],'.', clip_on = True,label='$L_c$=6.4',color='blue')
#
#ax.legend()
...
# #######################################################
# os.chdir('/home/lcbt87/BigSmall/RES')
# f=open(file='fpar',mode='r')
# fpar = f.readlines();
# dx = fpar[1].split(" ");dx = float(dx[0])
# dt = fpar[2].split(" ");dt = float(dt[0])
...
# ax.plot(rupxy[:,1],rupxy[:,0],':', clip_on = True,label='$L_c$=6.4', 
...
ax.set_title('3D homogeneous')
#
fig.savefig('../3d_0.2_0.05_6.4_homogeneous.svg')
#
